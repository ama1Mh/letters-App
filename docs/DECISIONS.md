...
| DEC-037 | **Hermes lacks `Intl.PluralRules` and other `Intl` APIs (2026-09-20, spike):** Arabic plurals cannot rely on the engine. Approach (polyfill vs legacy i18next plural format) is **proposed, not decided**: see OPEN-9 | Needs confirmation |
...
| OPEN-4 | **Phase 1 spike — partly done (2026-09-20, Android 35 emulator, dev build, Hermes, RN 0.86.3); findings below.** Still open: (a) native date-picker language (needs `@react-native-community/datetimepicker`, a native module + rebuild: owner approval needed), (b) interactive `TextInput` behaviour (typing, caret, keyboard switching, selection: needs a real keyboard, owner to try on `/spike`), (c) check on a physical phone before Phase 7 | End of Phase 1 |
| OPEN-9 | **`Intl.PluralRules` is missing in Hermes (DEC-037 below).** i18next needs it for Arabic plurals. Proposed fix: a JS polyfill (`@formatjs/intl-pluralrules` + `ar`/`en` locale data) loaded before i18n init, or i18next's legacy v3 plural JSON format. Not chosen yet: adds a dependency, so owner confirmation is needed. No pluralized strings exist yet, so nothing is broken today | Before the first pluralized string (Phase 3+) |
...
- **Hermes `Intl` is partial.** Available: `DateTimeFormat`, `NumberFormat`, `Collator`. Missing: `PluralRules`, `RelativeTimeFormat`, `ListFormat`, `Segmenter`, `Locale`, `DisplayNames`. Consequences: see OPEN-9 (plurals); no `Intl.RelativeTimeFormat` ("in 3 days") and no `Intl.ListFormat`, so those need our own code or a polyfill if the design calls for them.
...
- **Tooling (Windows, 7 GB RAM).** A local `x86_64` debug build takes ~1.5 h cold (mostly downloading Gradle 9.3.1), then is cached. The emulator (~1.8 GB) and Gradle cannot run together: build first, `./gradlew --stop`, then boot. Metro's first bundle takes ~3.5 min cold. If the app shows a black screen and logcat has `ProtocolException: Expected leading [0-9a-fA-F] character but was 0xd`, the first bundle download failed while Metro was still building: relaunch the app. Running `expo start` with `CI=1` turns file watching off (new routes and edits are then **not** served until Metro is restarted), and without it the bundle download failed repeatedly in this session, so the working routine was: restart Metro (`CI=1`) after edits, then launch/relaunch the app.
