...
| OPEN-4 | **Phase 1 spike — mostly done (2026-09-20, Android 35 emulator, dev build, Hermes, RN 0.86.3); findings below.** Still open: (a) interactive `TextInput` behaviour (typing, caret, keyboard switching, selection: needs a real keyboard, owner to try on `/spike`), (b) check on a physical phone before Phase 7. Native date/time picker: **no picker package is added until the scheduling UI needs one (owner, 2026-09-20)**; what could be observed without one is recorded below | End of Phase 1 |
