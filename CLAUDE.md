...
## Project status
**Phase 1 is done. Phase 2 (Auth & profiles) implementation is complete except two deliberately deferred items (Google Sign-In; password-reset completion — see DECISIONS.md DEC-040/OPEN-10). Phase 3 (Letters core: drafts) implementation is complete pending verification (DEC-041/OPEN-11). Phase 4 (Design system & renderer) is in progress.** `mobile/` has the auth flow, the local offline drafts store, the drafts repository (local/remote sync), a drafts list + compose screen with a live design preview, and the design picker; `supabase/` has three migrations (`profiles`, `letters`, `letters` design validation) and pgTAP tests, run in CI only (no local Docker — none of the three has been applied anywhere yet, including the owner's machine); `shared/design-catalog.json` is the first file in `shared/`. **Nothing has been verified on a device or emulator yet** — every screen and store above has RNTL test coverage only. Do not start Phase 5 until the owner says "go" for it. This directory is a git repository (working branch `dev`, remote `origin`).
