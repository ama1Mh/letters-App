...
## Project status
**Phase 1 is done. Phase 2 (Auth & profiles) implementation is complete except two deliberately deferred items (Google Sign-In; password-reset completion — see DECISIONS.md DEC-040/OPEN-10). Phase 3 (Letters core: drafts) implementation is complete pending verification (DEC-041/OPEN-11). Phase 4 (Design system & renderer) implementation is complete pending verification (DEC-042). Phase 5 (Discovery & invites) implementation is complete pending verification (DEC-043) — pgTAP CI is known-failing as of commit `66db8f2` and unverified for everything since (GitHub API unreachable in this environment; see DEC-043).** `mobile/` has the auth flow, the local offline drafts store, the drafts repository (local/remote sync), a drafts list + compose screen (recipient, design and live preview), the design picker, the recipient picker, the connection-requests inbox, the invite share/redeem screens and the privacy settings screen; `supabase/` has eight migrations (`profiles`, `letters`, `letters` design validation, rate limits, blocks/connections, invites, search, profile connection-visibility) and pgTAP tests, run in CI only (no local Docker — none has been applied anywhere yet, including the owner's machine); `shared/design-catalog.json` is the first file in `shared/`. **Nothing has been verified on a device or emulator yet** — every screen and store above has RNTL test coverage only. Do not start Phase 6 until the owner says "go" for it. This directory is a git repository (working branch `dev`, remote `origin`).
